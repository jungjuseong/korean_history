...
여기서 연산(…)은 인라인 프래그먼트를 만드는 데 사용됩니다. 이런 식으로 인터페이스를 구현하는 유형에서 필드를 가져올 수 있습니다.
...
지금까지 스칼라 유형을 인수로 사용했습니다. GraphQL을 사용하면 객체 유형을 mutation의 인수로 전달할 수도 있습니다. 유일한 차이점은 type 키워드를 사용하는 대신 input으로 선언해야 한다는 것입니다.
